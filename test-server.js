...
if (!TEST) {
    ml_front_config = require('/etc/ml-front-conf/mlfront-config.json');
    ...
    ml_front_config = require('./kube/test-ml/secrets/config.json');
...
app.use(express.static('./instances/' + ml_front_config.NAMESPACE + '/static/'));

app.set('view engine', 'pug');
var path = require('path');
app.set('views', path.join(__dirname, '/instances/' + ml_front_config.NAMESPACE));
...
app.get('/', async function (req, res) {
    ...
    if (req.session.loggedIn == undefined) {
        ...
        req.session.loggedIn = !ml_front_config.APPROVAL_REQUIRED;
        req.session.Title = ml_front_config.TITLE;
        req.session.plugins = ml_front_config.PLUGINS;
    };
